<template>
    <div v-click-outside="closeDropdown" class="relative mr-10">
        <button type="button" class="text-black-800 dark:text-white-0 text-sm leading-3 -tracking-sm font-bold flex justify-start items-center" @click="isOpen ? closeDropdown() : openDropdown()">
            Filter by status
            <span v-if="numberOfSelected" class="w-5 h-5 rounded-full text-xs bg-blue-500 text-white-0 flex justify-center items-center pt-1 ml-2">
                {{ numberOfSelected }}
            </span>
            <ArrowUpIcon :filled="true" :font-controlled="false" class="h-2 ml-3 mb-0.5" :class="!isOpen && '-scale-y-100'" />
        </button>
...
const numberOfSelected = computed(() => {
    const statuses = options.value.filter(status => invoicesStoreRef.filterBy.includes(status.name))
